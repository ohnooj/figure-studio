...
def format_debug_record(source: str, label: str, payload: object | None, client_timestamp: str | None = None) -> tuple[str, dict[str, object]]:
    iso_ts = datetime.now(UTC).astimezone().isoformat(timespec="milliseconds")
    record = {
...
def debug_log(payload: DebugLogPayload) -> dict[str, object]:
    if payload.events:
        for event in payload.events:
            print_debug_record(event.source, event.label, event.payload, event.client_timestamp)
        return {"ok": True, "count": len(payload.events)}
    print_debug_record(payload.source, payload.label or "unknown", payload.payload, payload.client_timestamp)
