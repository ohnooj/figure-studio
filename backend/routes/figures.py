...
from ..models import AssetImportPayload, FigureCreatePayload, FigureMetadataPayload, FigureSourceSavePayload
...
def create_figure(payload: FigureCreatePayload) -> dict[str, object]:
  return create_figure_entry(payload.id, payload.title, payload.templateId)
...
def import_figure_asset_route(figure_id: str, payload: AssetImportPayload) -> dict[str, object]:
  return import_figure_asset(figure_id, payload.sourcePath)


@router.post("/api/figure/{figure_id}/asset-upload")
async def upload_figure_assets_route(figure_id: str, files: list[UploadFile] = File(...)) -> dict[str, object]:
